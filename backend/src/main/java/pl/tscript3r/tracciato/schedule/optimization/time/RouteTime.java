...
public class RouteTime {
    ...
    public void onsite(StopDto stopDto) {
        var onsiteDuration = Duration.parse(stopDto.getOnsideDuration());
        if (willTravelerDoOvertime(onsiteDuration, Duration.ZERO))
            setNextDay(onsiteDuration, stopDto.getLocation(), false);
        else {
            futureNow = futureNow.plus(onsiteDuration);
            routeTimeline.onsite(futureNow, onsiteDuration, stopDto.getLocation());
        }
    }
